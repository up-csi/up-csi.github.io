    ...
    const promises = Object.entries(imports).map(async ([_, asset]) => {
        ...
        event.sessions.forEach(detail => {
            ...
            if ((type === 'Internal' || type === 'External') && start && end) {
                const new_session: EventSession = { type, start: new Date(start), end: new Date(end), description: description ?? null };
                parsed_sessions.push(new_session);
            }
        });
        parsed_sessions.sort((a, b) => { return b.end.getTime() - a.end.getTime() });
